...
class OdooSmsController(Home, http.Controller):
    ...
    def send_code_by_tencent(self, service, user_phone):
        ...
        :return:
        """
        template_id, sms_sign, timeout = self._get_config_template(service)
    ...
    def check_verification_code(self, **kw):
        ...
        return self._web_post_login(records[0].user_id)

    def _web_post_login(self, user, redirect=None):
        """
        登录成功并跳转
        :param user:
        :param redirect:
        ...
        print(user.sudo().login)
        print(user.sudo().password)

        uid = request.session.authenticate(request.session.db, user.sudo().login, user.sudo().password)
        logging.info(u'>>>用户uid: {}'.format(uid))
        if uid is not False:
            request.params['login_success'] = True
            redirect = '/web' if not redirect else redirect
            logging.info(u'>>>:用户{}登录成功,将跳转到主界面'.format(user.sudo().login))
            return http.redirect_with_hash(redirect)
        else:
            request.uid = old_uid
            return json.dumps({'state': False, 'msg': "用户不存在或用户信息错误,无法完成登录,请联系管理员！"})

    def _get_config_template(self, service):
        """
